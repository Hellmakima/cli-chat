...
import threading
import time
import keyboard  # requires admin on Windows; already in your deps
import requests
...
# Server endpoints
SERVER_URL = "http://https://sneaking-sook-persnickety.ngrok-free.dev"
WS_CHAT = "ws://https://sneaking-sook-persnickety.ngrok-free.dev/ws"
WS_VOICE = "ws://https://sneaking-sook-persnickety.ngrok-free.dev/ws/voice"
SERVER_URL = "http://localhost:8000"
WS_CHAT = "ws://localhost:8000/ws"
...
mute = False
push_to_talk = False
ptt_key = "space"  # hold to talk when push_to_talk=True
voice_running = False
voice_restart_requested = False

# Device indices (auto-detected)
...
# Helper: auto-detect Realtek devices
...
def audio_recorder_thread(loop, send_q: asyncio.Queue):
    ...
    # Try opening stream in blocking mode
    try:
        ...
        return  # recorder thread stops; voice remains disabled
    ...
    try:
        while voice_running:
            try:
                ...
                # On error, disable mic and stop recording loop
                mic_enabled = False
                print(f"⚠️ Audio read error: {e}. Voice send disabled.")
                break

            # data is ndarray shape (CHUNK, CHANNELS)
            if mute:
                continue
...
async def start_voice(room_id, username):
    ...
    if mic_index is not None:
        ...
        print(
            "⚠️ No Realtek mic found — voice send will be disabled unless default mic works"
        )
    ...
    recorder_thread = threading.Thread(
        target=audio_recorder_thread, args=(loop, send_queue), daemon=True
    )
    ...
    playback_thread = threading.Thread(
        ...
        args=(loop, play_queue, speaker_index),
        daemon=True,
    )
    ...
    try:
        async with websockets.connect(chat_url) as ws:
            ...
            while True:
                    ...
                    [send_task, recv_task, voice_task],
